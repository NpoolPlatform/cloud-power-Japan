export default {
  ...
  Warning: {
    ...
    P1: '当サイトのモバイル版は、ただいま開発中のため、接続が不安定になる可能性がございます.',
    P2: 'そのため、ユーザーの皆様には、PCブラウザ版でのログインやアカウント管理をお勧め致します。どうぞご理解とご協力のほどよろしくお願い申し上げます。',
  },
  ...
  Homepage: {
    ...
    Feature: {
      VCBacking: 'マーケットリーダー',
      VCBackingContent: 'Procyonのパートナーには、世界トップクラスのベンチャーキャピタルや世界最大のPoCマイニングプールが含まれています。長年の経験から、市場の動向を正確に把握し、綿密なデューデリジェンスを行い、今後のスーパースタープロジェクトをターゲットにして、Procyonコミュニティに商品や情報としてお届けしています。',
      TopTeam: 'ドリームチーム',
      TopTeamContent: 'Procyonの創業者たちやパートナー企業は、暗号通貨市場で10年以上の経験を持ち、投資利回りが100倍以上のプロジェクトに多数投資してきています。当社の技術チームは5年以上に渡り、1億6千万米ドル以上の資産を管理してきました。カスタマーサービス部門は、ハーフや帰国子女など、バイリンガルなメンバーで構成されており、英語と日本語での連絡が可能となっております。基本的に24時間以内に対応し、質の高い、優れたカスタマーサービスを提供することに力を注いでいます。',
      Technology: '全体像',
      TechnologyContent: '暗号通貨市場は、研究開発→VCファンド→マイナー→取引所と流れ、最終的にトークンが一般投資家に分配されます。Procyonでは、通常は機関投資家にしか提供されない機会を見つけ出すだけでなく、個人投資家にも機会を提供することを目指しています。',
    },

    Why: {
      Title: 'なぜProcyonを選ぶのか？',
      Content1: 'Procyonは、ワンストップの多次元な暗号資産グローバルマーケットプレイスの先駆けです。マイニングの',
      Content2: 'ファースト＆セカンドバッチやIDOプラットフォーム',
      Content3: 'など、暗号資産業界における',
      Content4: 'オープンな市場では見つけることができない独占的な投資機会', 
      Content5: '広範なネットワークと独自のリサーチ能力により、普通はアクセスすることができない独占的な機会をお届けします。',
      Content6: '日本は一人当たりの投資額がアジアで最も高い国です。日本市場は多くの',
      Content7: 'くのグローバルプレイヤーを魅了',
      Content8: 'していますが、日本には言語や文化の壁があり、グローバル市場から隔絶されているのが現状です。このハードルは、投資の機会損失や商品の多様性の欠如につながります。',
      Content9: 'Procyonは、皆様をグローバルな舞台へと導きます。',
    },

    What: {
      Title: 'ファーストバッチマイニングとは？',
      Content1: 'ファーストバッチマイニングは、メインネットが稼働すると同時に始まります。これは、第一陣のマイナーたちがメインネットでのローンチに先立ち、テストネットを通じて、すでにマイニング調整を完了させており、メインネット稼働と同時に開始されるということは、',
      Content2: 'マイニングの難易度が1番低いタイミングで開始することを意味します。真っ先にマイニングを開始することで、マイニング量が1番高い状態の時に参加することができ、結果としてより大きな投資利回りを得ることができます。',
      Content3: 'マイニングの投資利回りに影響を与える要素は2つあります。',
      Content4: 'マイニング量',
      Content5: 'トークン価格',
      Content6: 'マイニング量は、マイニングの難易度に直結します。マイナーが多いほど、ネットワーク全体のハッシュパワーの合計は大きくなり、マイニング難易度が上昇すると、マイナー1人当たりのマイニング量は比例して低くなります。マイニング可能な総供給量は基本的に固定されている為、マイニングに使用されるハッシュパワーが多くなればなるほど、ハッシュパワーの単位（TB）あたりのマイニング量は少なくなります。 ',
      Content7: '通常のマイニングに比べて、ファーストバッチマイニングのマイニング量は格段に多いです。 ',
      Content8: 'ピザをシェアするようなものです。多くの人とシェアすればするほど、1枚1枚のスライスが小さくなります。',
      Content14: 'Chiaというプロジェクトがありますが、Chiaのマイニングで事例を話すと、Chiaはメインネット開始後の2カ月からマイニングディフィカルティが急激に上がりました。結果、',
      Content15: 'その後のマイニングできる量が日に日に減っていきました。Chiaのファーストバッチマイニングに参加したマイナーは、メインネットが開始して最初の数日間で得たマイニング量が、最初の2カ月を過ぎてから参加したマイナーが1年間で得るマイニング量に相当しています。',
      Content16: 'これがファーストバッチの威力です。',
      Content9: 'トークンの価格自体は、市場の影響をダイナミックに受けます。通常、メインネットのローンチと',
      Content10: '取引所への上場は同時期に行われることが多く、需要と供給の関係から、注目されているプロジェクトであればあるほど、初期段階にトークンの価格が上昇する可能性が高いです。',
      Content11: '簡単に言うと、マイニングが早ければ早いほど、より多くのトークンを得ることができ、トークンが取引所に上場し、より高い価格でトークンを売ることができると、結果的に投資利回りが向上します。',
      Content12: '通常、ファーストバッチマイニングはプロのマイナーや機関投資家、もしくは経験豊富なマイニングプールでなければ参加することができません。Procyonは、個人投資家もファーストバッチマイニングに参加することを可能にし、より優れたタイミングでの投資機会を提供することを目指したハイエンドなプラットフォームです。',
  ...
  Mining: {
    ...
    Column2: {
      ...
      NetDailyOutput: 'ネットデイリーアウトプット（30日平均）:',
      TotalDaysMining: '総採掘日数:',
      Hashrate: 'ハッシュレート(30日平均):',
      NetworkDailyOutput: 'ネットワークデイリーアウト:',
  ...
  Wallet: {
    Title: 'アカウント金額',
    CurrentUSDT: 'カレントバランス(USDT)',
    CurrentBalance: 'カレントバランス(JPY)',
    Assets: {
      Title: '資産',
      ...
      Balance: 'バランス',
      Change: '24時間変更',
      MarketValueUSDT: '市場価値(USDT)',
      MarketValueBalance: '市場価値(JPY)',
      Transfer: '転送',
    },
    ...
    Guide: {
      Title: 'ガイド & FAQs',
      subtitle1: 'どのようにして資産を引き出すことができますか？',
      sub1: 'Assets Dashboardで、転送したいアセットの転送ボタンをクリックします。有効なウォレットアドレスを入力する必要があります。無効なアドレスを入力すると、お客様の資金は永久に失われます。',
      subtitle2: '取引にはどのくらいの時間がかかりますか？',
      sub2: '少額の取引（ネットワークによって決定される）の場合、取引は自動的に承認され、数分で完了しますが、ネットワークの混雑状況やガス料金によっては24時間かかる場合もあります。大口の取引（ネットワークによって決定される）の場合は、手動での処理が必要となり、その時点での取引量に応じて24時間以上かかることもあります。',
      subtitle3: 'スペースメッシュ・ウォーラーはどこで入手できますか？',
      sub3: 'SpacemeshはCoinbaseやBinanceなどの大手取引所に上場します。資金を送金する際には、必ずSpacemeshのウォレットアドレスを使用してください。誤ったアドレスを使用すると、SMH!',
    },
  },
  ...
  Account: {
    ...
    SecuritySetting: {
      ...
      Email: 'Email アドレス',
      EmailContent: 'Emailアドレスが設定されていることを確認ください。',
      EmailBtn: '確認',
      Mobile: '携帯電話の認証',
      MobileContent: '携帯電話の認証で、ログイン、入出金、その他セキュリティ設定の変更を行いましょう。',
      MobileBtn: '携帯電話の設定を変更',
      Google: 'Google認証',
      GoogleContent: '2段階認証を設定する事で、アカウントから入出金する際のセキュリティを強化する事ができます。',
      GoogleBtn: '2段階認証を設定',
      IDVerification: '身分証明の設定',
      IDVerificationContent: '身分を証明する画像付きの証明書をアップロードしましょう。',
      IDVerificationBtn: '身分証明書を設定',
      Verify: '設定済み',
      NotVerify: '設定を行う',
      Enable: '設定済み',
      NotEnable: '設定を行う',
      LoginVerification: 'ログイン時の認証',
      LoginVerificationContent: 'ログイン時の2段階認証を設定',
      GALogin: 'Google認証',
      Submit: '変更を申請',
  ...
  GoogleVerify: {
    Content: 'Google認証によるログイン認証を選択しました。認証コードをgoogle authenticatorに入力してください。',
  ...
  Notify: {
    ...
    GaVerify: {
      CantNull: '検証コードは空にできない',
      Success: 'google認証の確認に成功',
      Fail: 'google認証の確認に失敗する',
      FailToGetImg: 'google認証のQRコードが表示されない',
    },
    ...
    SubmitLoginVerify: {
      Success: 'google認証によるユーザーログインverfyの送信に成功',
    ...
    ForgetPassword: {
      Waiting: 'パスワード設定成功、ログイン待ち... ...',
      Success: '新しいパスワードの設定に成功',
      Fail1: 'パスワードと確認用パスワードが一致しない',
      Fail2: '新しいパスワードの設定に失敗する: ',
    },
  ...
  Faq: {
    ...
    Header1: {
      ...
      Q1: 'グーグルに出てきませんが、今回の為に設立されたLCCですか？出資者は大きな資本でしょうか。',
      A1: '暗号通貨業界と同様に、PRCN Technologyは比較的新しいベンチャー企業です。このような新しい分野の投資が吟味されるべきだとお感じになるのはご理解できます。しかし、弊社のチームには10年以上の業界経験があることをご理解ください。',
      Q2: '登記はオフショアでアフリカのセイシェルなんですね？優遇税制の為？',
      A2: '暗号通貨やブロックチェーンという新しい産業に対して、他の国よりも友好的な国があります。セーシェル共和国は、暗号通貨に優しい政策のために選ばれました。',
      Q3: 'サーバーが置いてあるのが香港、という事でしょうか。',
      A3: 'はい、弊社のサーバーは香港に置かれております。',
      Q4: '香港で洪水や停電が発生した場合、どうなりますか？私のマイニングマシンは大丈夫でしょうか？',
      A4: 'に記載されている通り、電気の信頼性ランキングが世界でもトップクラスです。',
      Diction: '香港は、香港電燈の公式サイト',
      Q5: '万が一、自然災害でサーバーが破損したり、強盗に遭ったりした場合は、Procyonが責任を持って補償してくれますか？',
      A5: '委託契約書6.7項に記載されている通り、台風などの制御不能な自然災害についての破損や損失は、当社で責任を負えません。強盗を防止するための責任については、お客様に対する義務の一環と認識しています。',
      Q6: '「技術サービス料」とは？',
      A6: '技術サービス料は、お客様のマイニングマシンの保守・運用コストを負担するものであり、お客様の日々のマイニング報酬から自動的に差し引かれます。',
    ...
    Header2: {
      ...
      Q1: 'Procyonプラットフォームと言うのはウォレットみたいな感じのものなんでしょうか？口座開設が必要なんでしょうか？',
      A1: 'はい。マイニング報酬はすべて、お客様のProcyonウォレットに直接入金されます。Procyonのウォレットは、お客様が当社のプラットフォームにアカウント登録をすることで自動的に口座開設されます。',
      Q2: 'マイニング報酬を売却したり、他の通貨に交換するにはどうすればいいですか？',
      A2: 'マイニング報酬はProcyonアカウントのウォレットに送金され、Procyonのウォレットからお好きな暗号通貨取引所のウォレットに移し、ご自由に売買することができます。',
      Q3: 'アカウントにGoogle認証をリンクしていて、携帯電話を紛失した場合はどうなりますか？',
      A3: 'ユーザーの皆様には、携帯電話の紛失や盗難の際にアカウントを復元できるよう、Googleバックアップコードをオフラインの安全な場所（紙など）に保存することを強く推奨します。ご自身でGoogle認証にアクセスできない場合は、',
      A4: 'にまでメールでお問い合わせ頂ければ、Google認証をアカウントから解除する手続きをご案内いたします。',
    },

    Header3: {
      Title: 'PAYMENTS, CONTRACTS, AND RECEIPTS',
      Q1: '契約期間はいつから始まりますか？サインをした日からですか？',
      A1: '契約期間は、お客様のマイニングマシンがマイニングを開始した日から始まります。',
      Q2: '契約書と領収書にはUSDTでの支払いとなっていますが、BTCで支払いました。これはなぜですか？',
      A2: 'BTCやその他の暗号通貨の価格は日々変動が激しいため、当社の販売パートナーは、価格が米ドルに固定され変動しづらい米ドルに接続された安定したコインUSDT（別名Tether）での支払いを変換しています。お客様のお支払いの安定性と正確性を保つ意図があります。お客様のお支払い金額に関する具体的なお問い合わせは、次のメールアドレスまでお問い合わせください。',
      Q3: '契約書に「TiB」という言葉が書かれていますが、その意味がよくわかりません。',
      A3: 'TiBはTerabyteの略であり、コンピューターのハードディスクの容量を表す単位です。ハードディスクを使ってマイニングする暗号通貨、例えばSpacemeshの場合、ストレージパワー（ストレージを使わない暗号通貨のマイニングではハッシュパワーとも呼ばれる）はTiBの単位で測定されます。',
      Q4: '暗号通貨については、あまり知りません。デジタル資産に関する日本の税法の仕組みを説明していただけますか？',
      A4: '日本の暗号通貨税法の詳細な内訳については、',
      A5: '以下のリンクをご参照してください。',
    },
    ...
    Header4: {
      ...
      A3: 'Spacemeshトークン（SMH）のための台帳を作成し、維持すること。Smeshは公正な暗号通貨であり、グローバルな決済手段です。',
      A4: 'SMHの公正で幅広い流通を確保する。',
      A5: 'トークン化されたアプリ、コミュニティ、経済のためのグローバルなプログラム可能なインフラを確立する。',
      Q2: 'Spacemeshの開発者たちは？',
      ...
      A13: 'SMHは、Coinbaseがこのプロジェクトに直接投資したため、Coinbaseに上場することは確実と言えるでしょう。また、HuobiやOKExにも上場される予定であり、Binanceにも上場される可能性が非常に高いと言えます。',
    },
    ...
    Header5: {
      ...
      A2: '金の採掘と同様に、調達（暗号通貨の場合はコンピューティングパワーやハードディスクスペース）には努力・労力が必要で、供給には限りがあり、マイニングされればされるほど、あるいは同時にマイニングしようとする人が多ければ多いほど、より多くマイニングすることが難しくなります（これを「マイニング難易度」といい、後述しています）。',
      Q2: 'マイニングプールとは？',
      A3: '「マイニングプール」とは、多数のマイニングマシンを組み合わせて、ブロックチェーン上の「ブロック」を発見する確率を高め、その結果、報酬として暗号通貨を獲得する大規模な暗号通貨マイニングのことです（以下で説明します）。',
      Q3: 'ブロックとは？',
  ...
  Legal: {
    ...
    hh11: '11. 連絡方法',
    pp13: '本プライバシーポリシーに対してのご質問がある場合、support@procyon.vipにお問い合わせください。',
